<div x-data wire:poll.15s>
    <div class="my-4 p-8 bg-gray-50 rounded-lg ring-gray-100 dark:bg-gray-950">
        <div class="flex flex-wrap items-center justify-between">
                        <div x-data="{showFullDate: false}" class="flex items-center gap-4">
                <x-filament::avatar
                        :src="$this->getAvatar()"
            ...
            <x-filament::link
                ...
                @if($this->comment->replies_count > 0)
                    <span title="{{ \Illuminate\Support\Number::format($this->comment->replies_count) }}">
                        {{\Illuminate\Support\Number::forHumans($this->comment->replies_count, maxPrecision: 3, abbreviate: true)}} {{ str('Réponse')->plural($this->comment->replies_count) }}
                    </span>
                @else
                    <span title="{{__('Aucune réponse pour l\'instant')}}">
                        Répondre
                    </span>
                @endif
            </x-filament::link>
    ...
    @if($showReplies)
        <div x-ref="repliesContainer" class="pl-8 border-l pb-4 border-b rounded-bl-xl my-2">
            ...
            <x-filament::icon-button
                    ...
                    "
                    type="button"
                    label="Masquer les réponses" icon="heroicon-o-minus-circle" class="absolute -left-8 -bottom-4" wire:click.prevent="toggleReplies"/>
        </div>
    @endif
</div>
